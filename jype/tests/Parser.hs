...
parserTests = defaultMain $ testGroup "parser"
    [ testCase "no description" $ common $
        [ Decl (TypeName "user" []) (Object [ Field "id" (ConcreteType "int" []) [] Nothing
                                            , Field "name" (ConcreteType "string" []) [] Nothing
                                            ]) []
        , Decl (TypeName "either" ["a", "b"]) (Choice [ Right (ConcreteType "a" [])
                                                      , Right (ConcreteType "b" [])
                                                      ]) []
        ]
    , testCase "declaration description" $ common $
        [ Decl (TypeName "id" ["a"]) (Choice [ Right (ConcreteType "a" []) ])
            [ "this is descriptions", "hogehoge", "yaayaa" ]
        ]
    , testCase "field description (before)" $ common $
        [ Decl (TypeName "id" ["a"]) (Object [ Field "a" (ConcreteType "a" [])
                                                 [ "hoge", "fuga", "moge" ] Nothing
                                             ]) []
        ]
...
common decls = case parseString (concatMap show decls) of
